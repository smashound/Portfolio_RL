...
class Env():
    ...
    def reset(self,start_para=None):
        # self.start = np.random.randint(0, self.full_horizon - 12000)
        if start_para is None:
            self.start = 10
        else:
            self.start = start_para
        while True:
            # print(self.full_state[:, :, self.start:self.start + self.horizon, :].shape)
            yield self.full_state[:, :, self.start:self.start + self.horizon, :]

            self.start += self.horizon+self.training_period*self.step_size
    ...
    def get_mu(self, vt, vt_1, wt_1, wt):
        ...
        wt_prime = (yt * wt_1) / (yt @ wt_1)
        mu = 0.8
        right = 1 / (1 - cp * wt[-1]) * (1 - cp * wt_prime[-1] - (cs + cp - cs * cp) * sum(
            np.maximum((wt_prime - mu * wt)[:-1].copy(), 0, (wt_prime - mu * wt)[:-1].copy())))
        ...
        # print('right',1 / (1 - cp * wt[-1]) * (1 - cp * wt_prime[-1] - (cs + cp - cs * cp) * sum(
        #     np.maximum((wt_prime - mu * wt)[:-1].copy(), 0, (wt_prime - mu * wt)[:-1].copy())))
        #       )
