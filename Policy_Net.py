...
class Policy_Net():
    ...
    def __init__(self, sess, feature_depth, num_asset, horizon, optimizer, depth1, depth2, sigma):
        ...
        with tf.variable_scope('input'):
            ...
            self.normalize_state_tensor = ((tf.div(self.state_tensor, self.state_tensor[:, :, -1:, :])) - 1) * 10
            # self.normalize_state_tensor = self.state_tensor
        ...
        with tf.variable_scope('policy_net'):
            ...
            out_size = in_size
            self.FC_input = tf.squeeze(self.conv3, [-1, -2]) * 10
            ...
            with tf.variable_scope('training'):
                ...
                ele = (
                -1 / 2 * tf.matmul(tf.matmul((self.actions - w_t), Sigma_inv), tf.transpose((self.actions - w_t))))
    ...
    def train_net(self, X, w, advantage, actions):
        _, loss = self.sess.run([self.train_op, self.surrogate_loss],
                                feed_dict={self.state_tensor: X, self.w_t_prev: w, self.advantage: advantage,
    ...
    def save(self):
        ...
        self.saver.save(self.sess, "./PolicyNet_Params/policy_net.cpkt")

    def restore(self):
        self.saver.restore(self.sess,
                           "/Users/william/Google Drive/STUDY/Columbia 2019 Spring/RL8100/Project/fin/Portfolio_RL/PolicyNet_Params/policy_net.cpkt")
...
if __name__ == '__main__':
    ...
    policy = Policy_Net(sess, feature_depth, num_asset, horizon, optimizer, depth1, depth2, sigma)
    ...
    sess.run(policy.train_op, feed_dict={policy.state_tensor: x, policy.w_t_prev: w, policy.advantage: advantage,
