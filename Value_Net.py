...
class Value_Net():
    ...
    def __init__(self, sess, optimizer, feature_depth, num_asset, horizon, depth1):
        ...
        with tf.variable_scope('input'):
            self.state_tensor = tf.placeholder(tf.float32, [None, self.num_asset,
                                                            self.horizon, self.feature_depth])
            self.normalize_state_tensor = ((tf.div(self.state_tensor, self.state_tensor[:, :, -1:, :])) - 1) * 10
            # self.w_t_prev = tf.placeholder(tf.float32,[None,self.num_asset])
    ...
    def get_state_value(self, state):
        return self.sess.run(self.fl, feed_dict={self.state_tensor: state})

    def train(self, state, target):
        _, loss = self.sess.run([self.train_op, self.state_loss],
                                feed_dict={self.state_tensor: state, self.target: target})
        print('state loss', loss)

    def save(self):
        if not os.path.exists("Model_Params"):
            os.mkdir("Model_Params")
        self.saver.save(self.sess, "./ValueNet_Params/value_net.cpkt")

    def restore(self):
        self.saver.restore(self.sess,
                           "/Users/william/Google Drive/STUDY/Columbia 2019 Spring/RL8100/Project/fin/Portfolio_RL/ValueNet_Params/value_net.ckpt")
