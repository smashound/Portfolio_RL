...
for xxx in range(10):
    ...
    sigma = [args.sigma] * num_asset

    Sigma = np.diag(sigma)

    policy = Policy_Net(sess, feature_depth, num_asset, horizon, optimizer_policy, depth1, depth2, sigma)
    value = Value_Net(sess, optimizer_value, feature_depth, num_asset, horizon, depth1=1)

    # initialize tensorflow graphs
    sess.run(tf.global_variables_initializer())
    start_date = np.random.randint(0,100)
    # main iteration
    for ite in range(iteration):
        ...
        for num in range(numtrajs):
            ...
            for i in range(max_train):
                # print('norm_tensor',sess.run(policy.normalize_state_tensor,feed_dict = {policy.state_tensor: obs, policy.w_t_prev:prev_acts[i]})[0,0,:,3])
                # print('-' * 100)
                # print(obs.shape)
        ...
        # print('VAL', VAL)
        # update policy
        # a, b, c, d, e, f = sess.run([value.state_tensor, value.normalize_state_tensor, \
        #                              value.conv1, value.conv2, value.FC_input, value.fl],
        #                             feed_dict={value.state_tensor: OBS})
        # print('state',a)
        # print('normal state', b)
        # print('conv1',c)
        # print('conv2',d)
        # print('fc',e)
        # print('fl',f)
        # print(sess.run([value.state_tensor,value.normalize_state_tensor,\
        # value.conv1,value.conv2,value.FC_input,value.fl],feed_dict = {value.state_tensor: OBS}))
